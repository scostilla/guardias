...
export class DdjjContrafacturaComponent implements OnInit, OnDestroy{
  ...
  /* ADAPTAR A CF, no tiene revista ni novedades */
  exportarAExcel() {
  ...
  getColor(tipoGuardia: TipoGuardia): string {
    if (tipoGuardia && tipoGuardia.id) {
      if (tipoGuardia.nombre === "CONTRAFACTURA") {
        return '#A9D08F'; 
      }
    }
  ...
  loadRegistrosMensuales(): void {
    ...
    this.registroMensualService.listByYearMonthEfectorAndTipoGuardiaCF(anio, mes, idEfector).subscribe(data => {
    ...
    this.dataSource.sort = this.sort;
      ...
      /* this.registrosMensuales = data.filter(registro =>
        registro.registroActividad.some(registroActividad =>
          registroActividad.tipoGuardia.nombre === "CONTRAFACTURA"
        )
      ); */
...
/* VER SI PONDERMOS COLOR YA QUE SOLO ES CF */
